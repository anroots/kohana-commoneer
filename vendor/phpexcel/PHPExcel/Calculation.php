        ...
        while (true) {
            ...
            if ($opCharacter == '-' && !$expectingOperator) { //	Is it a negation instead of a minus?
                ...
                while ($stack->count() > 0 &&
                       ...
                       @($operatorAssociativity[$opCharacter]
                               ? $operatorPrecedence[$opCharacter] < $operatorPrecedence[$o2['value']]
                               : $operatorPrecedence[$opCharacter] <= $operatorPrecedence[$o2['value']])) {
                ...
                if (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i', $val, $matches)) {
                    ...
                    if ($testPrevOp['value'] == ':') {
                        ...
                        if ((is_integer($startRowColRef)) && (ctype_digit($val)) &&
                            ...
                            //	Row range
                            $endRowColRef = (!is_null($pCellParent)) ? $pCellParent->getHighestColumn()
                            ...
                            //	Column range
                            $endRowColRef = (!is_null($pCellParent)) ? $pCellParent->getHighestRow()
        ...
        foreach ($tokens as $tokenData) {
            ...
            if (isset(self::$_binaryOperators[$token])) {
                ...
                if ($functionName != 'MKMATRIX') {
                    $this->_writeDebug('Evaluating Function ' . self::_localeFunc($functionName) . '() with ' . (($argCount == 0)
                                               ? 'no' : $argCount) . ' argument' . (($argCount == 1) ? '' : 's'));
                }
                ...
                if (isset(self::$_ExcelConstants[strtoupper($token)])) {
                    ...
                    $cellValue = $this->extractNamedRange($namedRange, ((null !== $pCell) ? $pCellParent
                                : null), false);
                    $pCell->attach($pCellParent);
